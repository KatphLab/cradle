...
describe('validateAgent', () => {
  ...
  it('rejects name with invalid characters', () => {
    const result = validateAgent(makeAgent({ name: 'TestAgent' }))
    expect(result.valid).toBe(false)
    expect(result.errors).toContain(
  ...
  it('rejects name starting with hyphen', () => {
    const result = validateAgent(makeAgent({ name: '-test' }))
    expect(result.valid).toBe(false)
  ...
  it('rejects name with consecutive hyphens', () => {
    const result = validateAgent(makeAgent({ name: 'test--agent' }))
    expect(result.valid).toBe(false)
    expect(result.errors).toContain(
  ...
  it('rejects name exceeding 64 characters', () => {
    const result = validateAgent(makeAgent({ name: 'a'.repeat(65) }))
    expect(result.valid).toBe(false)
    expect(result.errors).toContain('Name exceeds 64 characters (65)')
  })

  it('rejects description exceeding 1024 characters', () => {
    const result = validateAgent(makeAgent({ description: 'a'.repeat(1025) }))
    expect(result.valid).toBe(false)
    expect(result.errors).toContain(
